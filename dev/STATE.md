...
| Ask time-travel + token cost + aesthetic polish (2026-07-09) | Ask view reaches CLI parity on the TIME moat and the cost surface: an `as of` input → `ask_brain(as_of=…)` answers from the brain AS IT WAS at that instant (reads each hit's version current then, synthesizes only from those; empty-past reported honestly — verified live "nothing relevant as of 1990"), route marked `as-of`, an `as of <t>` badge on the answer; the per-answer token cost (already in `AskResult.context_tokens`) is labeled `cost`. Service gains `as_of` param + field; `/api/ask` passes `{question, as_of}`; CLI/MCP unchanged; TDD (past/empty-past/token tests). **Aesthetic pass** (lifts every view at once): one accent focus ring on all inputs/textareas/selects, themed placeholders, thin themed scrollbars. Remaining UX debt: Ask `--trace` detail (domains/candidates) not surfaced; Home cache-stale has no reindex action (needs a mutating endpoint); Review shows raw candidate IDs |
