#!/usr/bin/env python3
"""Hook Claude Code (SessionEnd/Stop): deposita la sessione di lavoro in Kortex.
...
`kortex remember`. È **non invasivo**: se `KORTEX_ROOT` non è impostato o non c'è un
transcript, non fa nulla. Imposta `KORTEX_ROOT` alla cartella del tuo brain.
"""
...
def main() -> int:
    brain = os.environ.get("KORTEX_ROOT")
    if not brain:
        return 0
    try:
    ...
    args = [sys.executable, "-m", "kortex.cli", "remember",
