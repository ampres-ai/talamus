# Talamus — Fondamenta Architetturali (la spina dorsale)
...
Riferimento: [visione di prodotto](2026-05-29-talamus-product-vision.md).

## 1. Mappa dei moduli

Il motore senza UI, i bordi che lo espongono, lo store che tiene la verità.

- **`talamus.model`** — tipi dati canonici. Solo dati, nessun I/O.
- **`talamus.store`** — l'**unico** a toccare il disco per la conoscenza: legge/scrive
  la verità (Markdown) e la cache derivata (`.talamus/`).
- **`talamus.adapters`** — i **giunti** (interfacce): converter, llm, search, ontology,
  renderer.
- **`talamus.engine`** — orchestrazione: ingest, extract, link, render, index,
  retrieve. Usa adapters + store.
- **`talamus.api`** (SDK) — le funzioni pubbliche. L'**unica vera API**.
- **bordi**: **`talamus.cli`** e **`talamus.mcp`** — involucri sottili sopra l'SDK.
...
- **Due rappresentazioni, due ruoli:**
  ...
  - **Oggetto canonico in `.talamus/cache/notes/<id>.json`** = la **verità "macchina"**:
    tiene i campi che l'umano non scrive a mano (provenienza/fonti, relazioni,
    `retrieval_text`, confidenza).
- **Riconciliazione esplicita con `talamus reindex`:** rilegge i `.md` e aggiorna la
...
- **Indici derivati** (grafo e ricerca: oggi file `graph.json` e `bm25.json` in
  `.talamus/cache/`; in futuro eventualmente SQLite/ontologia) sono **ricostruibili**
  da `reindex`/ingest.
- **Trade-off accettato:** se cancelli `.talamus/` perdi la provenienza (recuperabile
...
- **Layout di un "brain":** `notes/` in chiaro + `.talamus/` (grezzo, normalizzato,
...
- **CLI** = mappatura di questi verbi su `talamus ...`.
...
- verità ibrida: Markdown = vista umana, JSON in `.talamus/` = verità macchina,
