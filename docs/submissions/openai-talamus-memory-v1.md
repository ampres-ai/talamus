...
- **Requirement:** A local project with terminal access and either `uv` or an
  existing Talamus 1.1.2 executable.
...
```text
uvx --from "talamus==1.1.2" talamus demo --root <ABSOLUTE_FIXTURE_ROOT>
```
...
> consent-aware, CLI-first skill for Talamus 1.1.2 covering active-brain
