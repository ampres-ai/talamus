...
- The plugin launches `talamus[mcp]==1.1.2` through `uvx` for the active project.
...
```bash
uvx --from "talamus[mcp]==1.1.2" talamus scan . --dry-run --profile docs
```
...
```bash
uvx --from "talamus[mcp]==1.1.2" talamus export brain.zip
```
