...
- The plugin launches `talamus[mcp]==1.1.1` through `uvx` for the active project.
...
```bash
uvx --from "talamus[mcp]==1.1.1" talamus setup --capture ask
```
...
```bash
uvx --from "talamus[mcp]==1.1.1" talamus scan . --dry-run --profile docs
```
...
```bash
uvx --from "talamus[mcp]==1.1.1" talamus export brain.zip
```
