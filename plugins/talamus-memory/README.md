...
```bash
uvx --from "talamus[mcp]==1.1.2" talamus setup --capture ask
```

For a minimal brain without agent configuration or a capture hook:

```bash
uvx --from "talamus[mcp]==1.1.2" talamus init
```
...
the workspace. Do not run the installer itself through `uvx`: version 1.1.2
...
uvx --from "talamus[mcp]==1.1.2" talamus-mcp --help
```
